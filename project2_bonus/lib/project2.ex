defmodule Pro2 do
	def main(args) do
		...
		if nNum>1 do 
			...
			nFail = Enum.at(args,3) |> String.to_integer()
			Process.register(self(),:main)
			case algorithm do
				"gossip"->
					Pro2.gossip(nNum,topology,nFail)
					{time,_} = :timer.tc(fn -> waitSignal(nNum) end)
					IO.puts "Actual Time: #{time}"
					:timer.sleep(10)
				"push_sum"->
					Pro2.push_sum(nNum,topology,nFail)
	...
	def start(nNum,topology,algorithm,nFail) do
		Process.register(self(),:main)
		case algorithm do
			"gossip"->
				Pro2.gossip(nNum,topology,nFail)
	...
	#############################################algorithm
	def gossip(nNum,topology,nFail) do
    	IO.puts("gossip")
	...
	def push_sum(nNum,topology,nFail) do
    	IO.puts("push_sum")
    	list=Enum.to_list 1..nNum
    	plist=[]
